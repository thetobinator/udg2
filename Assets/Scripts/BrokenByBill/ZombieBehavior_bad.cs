...
/*
			...
			if (h != null) {
				...
				if (obj.GetComponent<HealthComponent> ().isDead ()) {
                   // Destroy(hc);
                    obj.tag = "Dead";
                    obj.name = "DeadBody";
                    obj.transform.parent = MainGameManager.DeadBodies.transform;
                    Destroy(z);     
				}
				result = true;
			}
	...
	bool dealDamage( GameObject human, float damage )
	{
		...
		{
            Debug.Log(human.tag);
            if (human.tag == "Human" || human.tag == "Zombie")
            {
                if (turnIntoRagdoll(human))
                {
                    if (human.GetComponent<ZombieBehavior>() == null)
                    {
            ...
            if (m_targetObject != null) {
				...
				if (dealDamage (m_targetObject, 50.0f)) {
                  switch (m_targetObject.tag) {
                        ...
                        case "Barricade":
                        Debug.Log("m_targetObject: " + m_targetObject);
                            break;
                        m_targetObject.GetComponent<HealthComponent>().explodeInstantly();
                        m_state = State.Alerted;
                        m_targetObject = null;
                    }
    ...
    /* with no collider component there's no collision.
...
void OnCollisionEnter(Collision collision) {
    ...
    {
        collision.gameObject.GetComponent<BreakableBarricades>().explodeInstantly();
